...
/**
 * The print-ready snapshot for a Delivery Note (M17 §7.2, series DN).
 *
 * The customer-facing record of what left the site — handed to the driver alongside the gate
 * pass. Assembled once, at print time, and stored verbatim in
...
export interface DispatchNoteSnapshot {
  ...
  readonly lines: readonly DispatchNoteLine[]
}

export async function loadDispatchNoteSnapshot(
  tx: Tx,
  dispatchOrderId: string,
): Promise<DispatchNoteSnapshot | undefined> {
  const headerRows = await rawRows(
    ...
    sql`
      select
        ...
        d.planned_quantity_kg, d.planned_kesha_count,
        d.gate_out_at, d.dispatched_at, d.notes
  ...
  return {
    ...
    loadedQuantityKg: col.numericOrNull(header.loaded_quantity_kg),
    loadedKeshaCount: col.intOrNull(header.loaded_kesha_count),
    plannedQuantityKg: col.numeric(header.planned_quantity_kg),
    plannedKeshaCount: col.intOrNull(header.planned_kesha_count),
    gateOutAt: col.dateOrNull(header.gate_out_at),
    dispatchedAt: col.dateOrNull(header.dispatched_at),
